...
class MailComposer(models.TransientModel):
    ...
    def _prepare_subject_with_prefix(self, wizard_mode, subject):
        ...
        Args:
            wizard_mode (Char): wizard mode
            subject (Char): subject

        Returns:
            Char: prepared subject with prefix
        """
        if not subject:
            return subject
        if wizard_mode == "forward":
            ...
            return subject
        # Convert the subject to lowercase to ensure case-insensitive comparison
        subject_lower = subject.lower()
        if not (
            subject_lower.startswith(prefix.lower())
            or subject_lower.startswith(prefix_str.lower())
        ):
