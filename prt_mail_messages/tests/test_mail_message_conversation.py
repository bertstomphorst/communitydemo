...
class TestMailMessageConversation(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.res_partner_test_1 = self.env["res.partner"].create(
            {
                "name": "Test Partner #1",
                "email": "test.partner@example.com",
            }
        )
        self.cetmix_conversation_1 = self.env["cetmix.conversation"].create(
            {
                "active": True,
                "name": "Test Conversation #1",
                "partner_ids": [Command.link(self.res_users_demo.partner_id.id)],
            }
        )
        self.cetmix_conversation_2 = self.env["cetmix.conversation"].create(
            {"name": "Test Conversation #2"}
        )
        self.mail_message_1 = self.env["mail.message"].create(
            {
                "res_id": self.cetmix_conversation_1.id,
                "model": self.cetmix_conversation_1._name,
                "reply_to": "test.reply@example.com",
                "email_from": "test.from@example.com",
                "body": "Mail message Body #1",
            }
        )
        self.mail_message_2 = self.env["mail.message"].create(
            {
                "res_id": self.cetmix_conversation_1.id,
                "model": self.cetmix_conversation_1._name,
                "reply_to": "test.reply@example.com",
    ...
    # TEST - 6 : Get conversations name get
    def test_conversations_display_name(self):
        """Get conversations display name"""

        # Default display_name: "Test Conversation #1"
        # display_name with context 'message_move_wiz':
        # "Test Conversation #1 - env.user.name"
        expected_result = "Test Conversation #1"
        result = self.cetmix_conversation_1.display_name
        self.assertEqual(result, expected_result, msg="Display names must be the same")
        result = self.cetmix_conversation_1.with_context(
            message_move_wiz=1
        ).display_name
        expected_result = f"Test Conversation #1 - {self.env.user.name}"
        self.assertEqual(result, expected_result, msg="Display names must be the same")
